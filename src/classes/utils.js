...
export default new class {
  ...
  stateVersionUp (originalState, currentState) {
    if (originalState.version !== currentState.version) {
      ...
      const versionUpTasks = (newVersion, id) => {
        ...
        for (const i in newVersionState.tasks[id]) {
  ...
  clone (original, current) {
    var out = {}
    for (const i in original) out[i] = original[i]
    for (const i in current) out[i] = current[i]
  ...
  widthResized (widthResized, cellWidth) {
    ...
    const more = widthResized - (cellWidth * count)
   ...
   */
  getDate (leftPositioin, cellWidth, tableStartDate) {
  ...
  random () {
    const l = 64
    // 生成する文字列に含める文字セット
    const c = '1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVZ'
    const cl = c.length
    let r = ''
    for (let i = 0; i < l; i++) {
